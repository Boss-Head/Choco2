...
param(
     # A URL para baixar o Chocolatey. Este padrao e o valor de
     # $env:chocolateyDownloadUrl, se estiver definido e, caso contrario, retornara ao
     # repositório oficial da comunidade Chocolatey para baixar o pacote Chocolatey.
     # Pode ser usado para instalacao offline fornecendo um caminho para um Chocolatey.nupkg.
    [Parameter(Mandatory = $false)]
    [string]
    $ChocolateyDownloadUrl = $env:chocolateyDownloadUrl,

     # Especifica uma versao de destino do Chocolatey para instalar. Por padrao, a ultima
     # versao estavel esta instalada. Isso usara o valor em
     # $env:chocolateyVersion por padrao, se essa variavel de ambiente estiver presente.
     # Este parâmetro e ignorado se -ChocolateyDownloadUrl estiver definido.
    [Parameter(Mandatory = $false)]
    [string]
    $ChocolateyVersion = $env:chocolateyVersion,

     # Se definido, usa ferramentas de descompactacao internas do Windows em vez de 7zip quando
     # descompactando o nupkg baixado. Isso sera definido por padrao se
     # $env:chocolateyUseWindowsCompression e definido com um valor diferente de 'false' ou '0'.
     #
     # Este parâmetro sera ignorado no PS 5+ em favor do uso do
    ...
    $UseNativeUnzip = $(
     ...
     # Se definido, ignora qualquer proxy configurado. Isso substituira qualquer proxy
     # variaveis ou parâmetros de ambiente. Isso sera definido por padrao se
     # $env:chocolateyIgnoreProxy e definido com um valor diferente de 'false' ou '0'.
    [Parameter(Mandatory = $false)]
    [switch]
    $IgnoreProxy = $(
     ...
     # Especifica o URL do proxy a ser usado durante o download. Isso sera padrao para
    ...
    $ProxyUrl = $env:chocolateyProxyLocation,
     ...
     # Especifica a credencial a ser usada para um proxy autenticado. Por padrao, um
     # credencial de proxy sera construida a partir de $env:chocolateyProxyUser e
     # $env:chocolateyProxyPassword variaveis de ambiente, se ambas estiverem definidas.
    [Parameter(ParameterSetName = 'Proxy', Mandatory = $false)]
...
function Get-Downloader {
  <#
     .SINOPSE
     Obtem um System.Net.WebClient que respeita proxies relevantes a serem usados para
     baixando dados.

     .DESCRIcaO
     Recupera um objeto WebClient pre-configurado de acordo com o especificado
     variaveis de ambiente para qualquer proxy e autenticacao para o proxy.
     As informacoes de proxy podem ser omitidas se o URL de destino for considerado
     ignorado pelo proxy (se origina da rede local.)

     .PARAMETER URL
     URL de destino que o WebClient estara consultando. Este URL nao e consultado por
     a funcao, e apenas uma referência para determinar se um proxy e necessario.

     .EXEMPLO
     Get-Downloader -Url $fileUrl

     Verifica se alguma configuracao de proxy e necessaria e/ou se $fileUrl
     e um URL que precisaria ignorar o proxy e, em seguida, gera o
     objeto WebClient ja configurado.
     #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $false)]
    ...
    if ($ProxyUrl) {
        ...
        $proxy.Credentials = if ($ProxyCredential) {
            ...
            Write-Warning "As credenciais padrao eram nulas e nenhuma credencial de proxy definida explicitamente foi encontrada. Tentando metodo de backup."
...
<#
     .SINOPSE
     Baixa o conteudo de um servidor remoto como uma string.

     .DESCRIcaO
     Baixa o conteudo da string de destino de um URL e gera a string resultante.
     Qualquer proxy existente que possa estar em uso sera utilizado.
     ...
     Recupera o conteudo dos dados de string no URL de destino e gera
     para a tubulacao.
     #>
    [CmdletBinding()]
    param(
...
function Set-PSConsoleWriter {
    ...
    try {
        ...
        Write-Warning "Nao foi possivel aplicar a correcao de redirecionamento."
    }
}
...
function Test-ChocolateyInstalled {
    ...
    elseif (-not (Get-ChildItem -Path $checkPath)) {
        ...
        Write-Warning "Arquivos de uma instalacao anterior do Chocolatey foram encontrados em '$($CheckPath)'."
        $true
    }
}
...
function Install-7zip {
    ...
    if (-not (Test-Path ($Path))) {
        Write-Host "Baixando 7-Zip ferramenta de linha de comando antes da extracao."
        Request-File -Url 'https://community.chocolatey.org/7za.exe' -File $Path -ProxyConfiguration $ProxyConfiguration

    }
    else {
        Write-Host "7zip ja presente, pulando a instalacao."
    }
}
...
if (Test-ChocolateyInstalled) {
    $message = @(
         "Uma instalacao existente do Chocolatey foi detectada. "
         "A instalacao nao continuara."
         "Por motivos de seguranca, este script nao substituira as instalacoes existentes."
         ""
         "Por favor, use 'choco upgrade chocolatey' para lidar com atualizacoes do próprio Chocolatey."
...
try {
    ...
    Write-Host "Forcar solicitacoes da Web para permitir TLS v1.2 (necessario para solicitacoes para Chocolatey.org)"
    [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072
}
catch {
    $errorMessage = @(
         'Nao e possivel definir o PowerShell para usar o TLS 1.2. Isso e necessario para entrar em contato com a Chocolatey a partir de 03 de fevereiro de 2020.'
         'https://blog.chocolatey.org/2020/01/remove-support-for-old-tls-versions/.'
         'Se você vir uma conexao subjacente fechada ou erros de confianca, talvez seja necessario fazer um ou mais dos seguintes procedimentos:'
         '(1) atualizar para .NET Framework 4.5+ e PowerShell v3+,'
         '(2) Chamar [System.Net.ServicePointManager]::SecurityProtocol = 3072; no PowerShell antes de tentar a instalacao,'
         ...
         '(4) use o metodo de instalacao Download + PowerShell.'
         'Veja https://docs.chocolatey.org/en-us/choco/setup para todas as opcoes de instalacao.'
    ) -join [Environment]::NewLine
...
if ($ChocolateyDownloadUrl) {
    if ($ChocolateyVersion) {
        Write-Warning "Ignorando -ChocolateyVersion parameter ($ChocolateyVersion) porque -ChocolateyDownloadUrl ja esta definido."
    }

    Write-Host "Baixando Chocolatey de: $ChocolateyDownloadUrl"
} elseif ($ChocolateyVersion) {
    Write-Host "Baixando uma versao especifica do Chocolatey: $ChocolateyVersion"
    $ChocolateyDownloadUrl = "https://community.chocolatey.org/api/v2/package/chocolatey/$ChocolateyVersion"
} else {
    Write-Host "Obtendo a versao mais recente do pacote Chocolatey para download."
...
if ($PSVersionTable.PSVersion.Major -lt 5) {
    ...
    if ($UseNativeUnzip) {
        Write-Host 'Usando compactacao integrada para descompactar'
        ...
        try {
            ...
            Write-Warning "Nao e possivel descompactar o pacote usando a compactacao integrada. Defina `$env:chocolateyUseWindowsCompression = ''` ou omita -UseNativeUnzip e tente novamente usar 7zip para descompactar."
            throw $_
        }
    } else {
        ...
        $errorMessage = "Nao e possivel descompactar o pacote usando 7zip. Talvez tente configurar `$env:chocolateyUseWindowsCompression = 'true' e chame install novamente. Erro:"
        if ($exitCode -ne 0) {
...
Write-Host "Instalando o Chocolatey na maquina local"
